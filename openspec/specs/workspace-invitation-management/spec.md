...
by entering the invited user's email address.

#### Scenario: Authorized member creates invitation
- **WHEN** a workspace member with invitation-create permission submits an email in the create-invitation modal
- **THEN** the system creates a pending invitation for the current workspace with the default `member` role
...
#### Scenario: Duplicate or redundant invitation is rejected
