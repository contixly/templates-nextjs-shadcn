...
Therefore no framework configuration or source migration is planned. The new navigation and Turbopack capabilities remain unenabled: enabling experimental/optional performance features would change runtime behaviour and requires separate profiling and product validation.
...
3. Review the manifest and lockfile diff for accidental major-version changes or unrelated file modifications. If updated package APIs cause a failure, make the smallest compatible source adjustment and record why.
4. Run, in order, `npm run lint`, `npm test`, `npm run build`, and `npm run e2e`. A command must exit successfully before the upgrade is accepted.
